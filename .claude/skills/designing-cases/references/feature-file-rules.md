...
Everything belonging to one feature lives together: feature files, feature-specific step definitions, and schema.
...
└── <feature-name>/              ← e.g. dual-analyst, player-profile, match-history
    ...
    ├── performance.feature      ← only if performance AC is explicitly in the ticket
    ├── schemas/
    │   └── <endpoint>.js        ← request + response schema for this feature
    └── step-definitions/        ← steps unique to this feature only
        ├── [feature]-steps.js
        ├── hallucination-steps.js  ← if hallucination.feature present
        └── schema-steps.js         ← uses schemas/<endpoint>.js
...
**Performance tests:** Only create `performance.feature` and its step definitions if performance AC is explicitly stated in the ticket. If no performance AC — skip entirely. Performance load tests run via k6, not Cucumber.

**Rule: check shared/ before writing any new step. If a step can apply to any feature, it belongs in shared/. Only create a feature-specific step if it is genuinely unique to this feature and cannot be reused.**
