...
└── <feature-name>/              ← e.g. dual-analyst, player-profile, match-history
    ...
    └── performance.feature      ← baseline + load (if performance AC present)
```
...
Skip `performance.feature` if there is no performance AC.

Step definitions follow the same pattern — shared steps go in shared/, feature-specific steps go in their own directory:
...
└── <feature-name>/              ← specific to this feature only
    └── [feature]-steps.js       ← steps that cannot be reused elsewhere
```
